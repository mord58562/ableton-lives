...
```
~/Downloads/ableton-lives/
  bin/
    lives-watch.sh          # launchd-spawned script: filter, dedup, copy, log, prune, emit summary
    lives-restore.sh        # CLI restore: list or copy one version back (called by Quick Action)
    lives-prune.sh          # retention pruner: keep-all-30d, 1-per-day-6mo, delete older
  launchd/
    com.mord58562.ableton-lives.watch.plist      # file watcher (WatchPaths, KeepAlive)
    com.mord58562.ableton-lives.prune.plist  # daily 04:00 pruner
  quickaction/
...
Logs and summary:
```
~/Library/Logs/ableton-lives.log   # appended by lives-watch.sh; rotated weekly
~/Documents/.ableton-lives-summary                  # machine-readable summary for digest consumers
```
...
```xml
Label: com.mord58562.ableton-lives.watch
ProgramArguments: [/bin/zsh, ~/Downloads/ableton-lives/bin/lives-watch.sh]
WatchPaths:
...
ThrottleInterval: 5
RunAtLoad: false
StandardOutPath: ~/Library/Logs/ableton-lives.log
StandardErrorPath: ~/Library/Logs/ableton-lives.log
```

`ThrottleInterval: 5` is launchd's minimum coalescing window. If Ableton saves twice within 5 seconds (autosave + manual save), launchd delivers one wakeup. The script compensates by scanning with a 30-second mtime window (D2) so neither save is lost. `KeepAlive: true` means launchd restarts `lives-watch.sh` on crash and on login - the script is intentionally short-lived (runs, copies, exits), so KeepAlive causes it to re-arm after each fire. That is correct behavior; the script must not loop internally.
...
Ableton Lives does .als versioning only. The sets/ directory (20.3 GB, 57 irreplaceable WAV recordings) is out of scope here. Reasons: different cadence (recordings happen occasionally, not on every Ableton save), different copy strategy (rsync mirror, not timestamped versions), different size budget (20 GB vs <2 GB for .als versions). A sister project - provisionally named "usb-mirror" - handles sets/ via a one-time rsync to internal disk plus an incremental watcher. Its recommended layout is shown in Section 8.

Keeping the scopes separate means each project has a single, clear invariant. Ableton Lives' invariant: "every .als save is in _versions/ within 30 seconds, with retention enforced." Mixing in 20 GB WAV mirroring would blur that invariant and complicate the prune logic.

### D4 - Retention pruning runs daily at 04:00

A second launchd plist - `com.mord58562.ableton-lives.prune.plist` - fires `lives-prune.sh` at 04:00 daily (StartCalendarInterval: Hour=4, Minute=0). The pruner does not piggyback on saves because (a) prune is I/O-heavy and should not slow the save-time copy path, and (b) 04:00 is when Ableton is not running and the machine is idle.

Retention policy applied by `lives-prune.sh`:
...
3. Calls `lives-restore.sh --list "<project-name>"` to get the sorted list of available timestamps.
4. Passes that list to an AppleScript `choose from list` dialog.
5. Calls `lives-restore.sh --restore "<project-name>" "<chosen-timestamp>" "<destination-path>"` to copy the version back.

`lives-restore.sh --restore` writes to a `.als.bak` file alongside the original before overwriting, so a single undo-level is always available without Ableton Lives infrastructure.

AppleScript pseudo-flow:
```
set versions to paragraphs of (shell script "lives-restore.sh --list ...")
...
shell script "lives-restore.sh --restore ... " & quoted form of item 1 of chosen
...
If name collision becomes real: the mitigation is to prefix with a single letter for the source (`u-` for USB2, `i-` for internal). That is a one-line change to `lives-watch.sh`.

### D8 - Log rotation

`~/Library/Logs/ableton-lives.log` is appended by `lives-watch.sh` and `lives-prune.sh`. Rotation is handled by a weekly launchd timer (added to the prune plist as a secondary StartCalendarInterval) that runs `newsyslog`-style rotation in bash: if the log exceeds 5 MB or is older than 7 days, move it to `ableton-lives.log.1` (overwriting any prior `.log.1`) and start a fresh log. No `logrotate` dependency - bash `stat` gives file size; `find -mtime +7` gives age. The rotation runs at 04:05 daily (5 minutes after the pruner) so log and prune output from the same night land in the same file before rotation.

### D9 - Digest summary for downstream consumers

After every successful version copy, `lives-watch.sh` updates `~/Documents/.ableton-lives-summary` with:

```
LIVES_LAST_UPDATED=2026-05-12T04:23:11
LIVES_VERSION_COUNT_TOTAL=47
LIVES_STORAGE_BYTES=19234816
LIVES_RECENT_SAVES=first beat|2026-05-12T04:23:11,chord progression|2026-05-11T18:22:05
LIVES_PRUNE_LAST_RUN=2026-05-12T04:00:03
LIVES_PRUNE_DELETED_LAST_RUN=3
```

Format: shell-sourceable KEY=VALUE pairs. This lets any digest script (`source ~/.ableton-lives-summary`) read current state without parsing logs. A Filename Court Sunday digest or any future dashboard can consume this without touching Ableton Lives' internals. The file is written atomically (write to `.ableton-lives-summary.tmp`, then `mv`).

---

## 3. Watcher Script Logic (lives-watch.sh)
...
1. **Guard: single instance.** Check for a lockfile at `/tmp/ableton-lives-watch.lock`. If present and the PID inside is still alive, log "already running" and exit 0. Write own PID to lockfile.
...
3. **Prune stale seen-hashes entries.** Read `~/.ableton-lives-seen-hashes`. Drop lines whose date field is not today. If any lines were dropped, rewrite the file.
...
5. **For each candidate .als file:**
   ...
   c. Check if hash appears in `~/.ableton-lives-seen-hashes`. If yes, log "dedup skip" and continue.
   ...
   h. Record hash + today's date in `~/.ableton-lives-seen-hashes`.
   i. Log: `[COPIED] <project>/<basename>-<timestamp>.als (<size> bytes)`.

6. **Update `.ableton-lives-summary`.** Recount total versions and storage bytes. Prepend the two most recent saves to the RECENT_SAVES field. Write atomically.
...
on run {input, parameters}
    ...
    -- get sorted version list from lives-restore.sh
    set version_list to paragraphs of (do shell script \
        "~/Downloads/ableton-lives/bin/lives-restore.sh --list " & quoted form of project_name)
    ...
    do shell script "~/Downloads/ableton-lives/bin/lives-restore.sh --restore " \
        & quoted form of project_name & " " \
        & quoted form of chosen_version & " " \
        & quoted form of als_path

    display notification "Restored " & chosen_version with title "Ableton Lives"
end run
```

`lives-restore.sh --list <project>` prints one timestamp per line, newest first, format `YYYYMMDD-HHMMSS`, so the list dialog shows most-recent at top. `lives-restore.sh --restore` backs up the current file to `<original>.bak`, then copies the versioned file to the destination path.
...
1. Verify `~/Downloads/ableton-lives/bin/*.sh` are executable; chmod if not.
...
7. On smoke pass: open the log file with `open ~/Library/Logs/ableton-lives.log` (satisfies feedback_open_after_install - something tangible opens without the user having to ask).
8. Print a one-line summary: "Ableton Lives installed. Watching USB2 ableton/ and ~/Music/Ableton/. Versions -> ~/Music/Ableton/_versions/. Log open."

### uninstall.sh

1. Unload and remove both launchd plists.
2. Remove the Automator workflow from `~/Library/Services/`.
3. Remove `~/.ableton-lives-seen-hashes` and the lockfile if present.
4. Print: "Ableton Lives uninstalled. Version store at ~/Music/Ableton/_versions/ is UNTOUCHED. Delete manually if desired."
...
2. **Act (version 1)**: manually invoke the copy logic (sourcing the relevant function from `lives-watch.sh`, or calling it directly with overridden `VERSIONS_DIR`). Assert a versioned copy appears in the temp `_versions/`.
...
5. **Restore**: invoke `lives-restore.sh --restore` pointing at the first timestamp. Assert the destination .als SHA-256 matches the original (step 1).
...
- **exits_0_when_usb2_missing**: set `USB2_PATH` to a nonexistent path. Run `lives-watch.sh`. Assert exit code 0.
...
6. If not found within 10 seconds: print "SMOKE FAIL - no version copy detected. Check ~/Library/Logs/ableton-lives.log." Exit 1.
...
Note: the smoke test depends on launchd having loaded the plist (step 4 in install.sh). If launchd has not yet fired `lives-watch.sh`, the smoke test uses the 10-second window to allow for the ThrottleInterval. In practice launchd fires within 1-3 seconds of a WatchPaths event.

---

## 8. Sister Project: sets/ Backup

Ableton Lives does not touch sets/. The sister project - working name `usb-mirror` - handles it. Recommended layout:
...
~/Downloads/usb-mirror/
  ...
  launchd/
    com.mord58562.ableton-lives.usb-mirror.plist     # fires on USB2 mount via DiskArbitration hook
  scripts/
    install.sh
    uninstall.sh
  README.md
...
PIONEER/ and User Library/ backups would also live in usb-mirror or as separate simple rsync jobs, not in Ableton Lives.
...
| _versions/ grows beyond 71 Gi free | Very low (budget math shows <2 GB / 6 months) | Medium | Pruner + storage alerting in .ableton-lives-summary (LIVES_STORAGE_BYTES); add a check to smoke.sh if desired |
...
| lives-restore.sh overwrites .als with wrong version | Low (user-confirmed dialog) | High | .bak file created before every restore; one undo level always present |
...
There are 9 backup files already there (firtsecond [2026-05-08 003805].als etc.) with known timestamps. A one-time import script could ingest them into the version store with the correct timestamps. Recommended default: yes, include a `bin/lives-import-existing.sh` step in install.sh so history is not lost.
